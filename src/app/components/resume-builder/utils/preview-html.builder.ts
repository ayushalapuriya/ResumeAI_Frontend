...
// preview pane.  No Angular dependencies – easy to unit-test in isolation.
// ─────────────────────────────────────────────────────────────────────────────
...
function buildProjectsHtml(data: ResumeData): string {
...
function buildPreviewStyles(accentColor: string): string {
  return `
    <style>
...
//
// Naming convention: buildTemplate<Section>Html(data)
// ═════════════════════════════════════════════════════════════════════════════

/**
 * Builds the {{CONTACT}} block for templates that use the icon-row layout:
 *   .s-contact-item  →  .s-contact-icon (inline SVG)  +  .s-contact-text
 *
 * Used inside .h-contact (header-left column) in templates like "Formal Resume".
 */
function buildTemplateContactHtml(data: ResumeData): string {
  ...
  const items: { svg: string; text: string }[] = [];
...
/**
 * Builds the {{EXPERIENCE}} block using the timeline-dot layout:
 *   .exp-item  →  .exp-dot-col (.exp-dot + .exp-line)  +  .exp-content
 *     .exp-content  →  .exp-meta (.exp-role, .exp-company, .exp-date)  +  .exp-desc
 *
 * The vertical .exp-line connector is omitted on the last item so the
 * timeline terminates cleanly without a dangling line.
 */
function buildTemplateExperienceHtml(data: ResumeData): string {
...
/**
 * Builds the {{EDUCATION}} block using the two-column card layout:
 *   .edu-item  →  left (.edu-degree, .edu-field, .edu-inst)  +  .edu-right (.edu-dates)
 */
function buildTemplateEducationHtml(data: ResumeData): string {
...
/**
 * Builds the {{PROJECTS}} block using the left-border card layout:
 *   .proj-card  →  .proj-top (.proj-name + .proj-url)  +  .proj-role
 *                  + .proj-desc  +  .proj-techs (.tech-pill for date range)
 */
function buildTemplateProjectsHtml(data: ResumeData): string {
...
/**
 * Builds the {{SKILLS}} block using the animated progress-bar layout:
 *   .skill-bar-wrap  →  .skill-bar-label  +  .skill-bar-track → .skill-bar-fill
 *
 * Bar widths cycle through a set of visually balanced percentages so the
 * sidebar never looks uniformly flat when the user has not rated skills.
 */
function buildTemplateSkillsHtml(data: ResumeData): string {
  if (!data.skills.length) {
    return '<p class="empty-section">No skills added yet.</p>';
  }

  // Visually varied widths that feel natural — not uniform, not random.
...
/**
 * Generates the full preview HTML for the built-in (fallback) template.
 * Returns a raw HTML string; the caller is responsible for sanitising it before
 * binding with [innerHTML].
 */
export function buildUniversalPreviewHtml(data: ResumeData, accentColor: string): string {
  ...
  return `
    ...
    <div class="rv-wrap">
      ...
      <div class="rv-body">
        ...
        ${data.experience.length ? `<div class="rv-section"><div class="rv-sec-label">Experience</div>${exp}</div>`   : ''}
        ${data.education.length  ? `<div class="rv-section"><div class="rv-sec-label">Education</div>${edu}</div>`   : ''}
        ${data.projects.length   ? `<div class="rv-section"><div class="rv-sec-label">Projects</div>${proj}</div>`   : ''}
        ${data.skills.length     ? `<div class="rv-section"><div class="rv-sec-label">Skills</div><div class="rv-skills-wrap">${skills}</div></div>` : ''}
      </div>
    </div>`;
}

/**
 * Injects resume data into a template's htmlLayout via {{placeholder}} replacement.
 *
 * Handles both scalar fields and rich section blocks:
 *
 *   Scalar fields  →  plain text substitution
 *   {{CONTACT}}    →  icon-row HTML  (.s-contact-item / .s-contact-icon / .s-contact-text)
 *   {{EXPERIENCE}} →  timeline-dot HTML  (.exp-item / .exp-dot / .exp-content / …)
 *   {{EDUCATION}}  →  two-column card HTML  (.edu-item / .edu-degree / .edu-right / …)
 *   {{PROJECTS}}   →  left-border card HTML  (.proj-card / .proj-top / .proj-desc / …)
 *   {{SKILLS}}     →  progress-bar HTML  (.skill-bar-wrap / .skill-bar-track / …)
 *
 * Returns a raw HTML string prefixed with the template's cssStyles.
 * The caller is responsible for sanitising it before binding with [innerHTML].
 */
